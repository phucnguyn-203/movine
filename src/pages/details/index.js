...
const Details = () => {
    ...
    useEffect(() => {
        const getDetails = async () => {
            try {
                const detailsMovie = await axios.get(`${process.env.REACT_APP_API_ENDPOINT}/${mediaType}/${id}?api_key=${process.env.REACT_APP_API_KEY}`);
                ...
                <div ref={bannerRef} className='banner' style={{ backgroundImage: `url(${process.env.REACT_APP_IMAGE_ENDPOINT}${details?.backdrop_path})` }}>
                    <div className='movie__content'>
                        <div className='movie__poster'>
                            <img src={`${process.env.REACT_APP_IMAGE_ENDPOINT}${details?.poster_path}`} alt="poster" />
                        </div>
                        ...
                        <div className='movie__info'>
                            ...
                            <p className='movie__release-date'>{
                                details?.release_date ? `Release date: ${details?.release_date}` : `Last episode: ${details?.last_air_date}`
                            }</p>
                            ...
                            <div>
                                <Link to={mediaType === 'movie' ? `/watch/${mediaType}/${id}` : `/watch/${mediaType}/${id}/season/1/esp/1`} className='button'>Watch Now</Link>
