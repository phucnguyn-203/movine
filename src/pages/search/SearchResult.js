...
const SearchResult = () => {
    ...
    const getSearchMovie = async () => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_API_ENDPOINT}/search/multi?api_key=${process.env.REACT_APP_API_KEY}&query=${searchKeyWord.trim()}&page=${page}`);
        ...
        <Layout>
            {isLoading ? <Loading /> : <div style={{ marginTop: 80 }}>
                {searchMovie.length !== 0 ? <><h1 style={{ color: "#FFF", textAlign: "center" }}>Search Result For {searchKeyWord}</h1>
                    ...
                    : <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: `calc(100vh - 7.2px)` }}>
                        <h1 style={{ color: "#FFF", textAlign: "center" }}> No Search Result For {searchKeyWord}</h1>
                    </div>
                }
            </div>
            }
